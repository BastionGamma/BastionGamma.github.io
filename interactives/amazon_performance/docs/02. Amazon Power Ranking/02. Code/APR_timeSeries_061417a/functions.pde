...
void fetchRanks() {
  msql = new MySQL( this, "localhost", database, user, pass );
  if (msql.connect()) {
    msql.query(queryRanks);
    while ( msql.next () ) {  
      brandRank.add(msql.getString(1), msql.getFloat(2));
    }
  }
    brandRank.sortValuesReverse();
    brandRankArray = brandRank.keyArray();
    brandRankValue = brandRank.valueArray();
    println(brandRank);
}
