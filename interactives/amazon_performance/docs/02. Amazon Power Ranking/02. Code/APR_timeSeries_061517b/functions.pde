...
void fetchBrands() {
  brands = new String[0];
  queryBrands = "select brand, avg(distinct score_7) as s from " + tables[table] + " where month(date)=" + month + " and year(date)=" + year + " group by brand order by s desc";
...
void initPlots() {
  queryScores = "select dayofyear(date), score_7 from " + tables[table] + " where brand=\'temp\' and score_7 is not null and month(date)=" + month + " and year(date)=" + year;
...
void checkVariability(int j, float sd_scores[]) {
  ...
  println("Most variable brand: " + variableBrand);
}
...
void labelTable() {
  ...
  textSize(20);
  textAlign(LEFT);
  text(tables[table], margin*2, margin/2);
}
...
void keyPressed() {
  if (key == '=' && selectedBrand<plotList.size()-1) {
    ...
    selectedBrand--;
  } else if (key == '.' && table<tables.length-1) {
    table++;
    fetchBrands();
    initPlots();
